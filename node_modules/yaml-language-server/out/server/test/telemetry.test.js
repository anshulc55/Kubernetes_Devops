...
describe('Telemetry Tests', () => {
    ...
    let telemetry;
    beforeEach(() => {
        const telemetryInstance = new telemetry_1.Telemetry({});
    ...
    describe('Kubernetos schema mapping', () => {
        it('should not report if schema is not k8s', () => {
            schemaUrls_1.checkSchemaURI([], vscode_uri_1.URI.parse('file:///some/path'), 'file:///some/path/to/schema.json', telemetry);
            expect(telemetry.send).not.called;
        });
        it('should report if schema is k8s', () => {
            schemaUrls_1.checkSchemaURI([], vscode_uri_1.URI.parse('file:///some/path'), 'kubernetes', telemetry);
